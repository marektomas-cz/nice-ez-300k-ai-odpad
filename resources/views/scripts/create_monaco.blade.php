                        ...
                        <div class="script-editor-container">
                            <div class="editor-toolbar">
                                ...
                                <div class="editor-status">
                                    ...
                                    <span id="editorPosition" class="text-muted">Line 1, Column 1</span>
                                </div>
                            </div>
                            <div id="monacoEditor" class="monaco-editor-container" style="height: 400px; border: 1px solid #dee2e6;"></div>
...
document.addEventListener('DOMContentLoaded', function() {
    scriptEditor = new ScriptEditor('monacoEditor', {
    ...
    scriptEditor.editor.onDidChangeModelContent(() => {
        document.getElementById('code').value = scriptEditor.getValue();
    });

    // Load initial content if any
    const initialCode = document.getElementById('code').value;
    if (initialCode) {
        scriptEditor.setValue(initialCode);
    }
});
...
const users = await api.database.select('users', ['id', 'name', 'email'], {
    active: true
});
...
const response = await api.http.get('https://jsonplaceholder.typicode.com/posts/1');
...
await api.events.dispatch('script.custom.notification', eventData);
...
for (const user of parsedUsers) {
    await api.events.dispatch('user.notification', {
        user_id: user.id,
...
function formatCode() {
    scriptEditor.editor.getAction('editor.action.formatDocument').run();
}

function validateSyntax() {
    scriptEditor.validateSyntax();
}

function insertTemplate() {
    scriptEditor.showTemplateMenu();
}

function toggleTheme() {
    currentTheme = currentTheme === 'vs-dark' ? 'vs' : 'vs-dark';
    scriptEditor.setTheme(currentTheme);
}
...
function loadTemplate(templateKey) {
    ...
    if (template) {
        ...
        scriptEditor.setValue(template.code);
...
function showValidationResults(data) {
    ...
    if (data.ast_analysis) {
        ...
        html += '<strong>Security Score: ' + data.ast_analysis.score + '/100</strong><br>';
        if (data.ast_analysis.issues && data.ast_analysis.issues.length > 0) {
            html += '<small>AST Issues:</small><ul class="mb-0">';
            data.ast_analysis.issues.forEach(issue => {
                html += '<li><small>' + issue.message + ' (Severity: ' + issue.severity + ')</small></li>';
            });
            html += '</ul>';
        }
        html += '</div>';
    ...
    if (draft && !document.getElementById('name').value) {
        try {
            ...
            if (confirm('Load saved draft?')) {
                ...
                if (scriptEditor && draftData.code) {
                    scriptEditor.setValue(draftData.code);
                }
            }
        } catch (e) {
